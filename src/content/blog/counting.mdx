...
The number of ways we can sample (pick) $$ n $$ objects $$ k $$ times is simply $$ n^k $$. 
Sampling with replacement can be thought of as filling $$ k $$ slots in a bag from a pool of $$ n $$
objects (constant) where each slot gets $$ n $$ choices.
When a dice is rolled twice, there are a total of $$ 36 $$ different outcomes. This is because for both 
rolls, we have a total of $$ 6 $$ possible outcomes (die rolls are independent). The combined outcomes
...
and $$ 10 $$ digits ($$ 0 $$ to $$ 9 $$), with no restrictions for repetition. Since there are
$$ (26 + 10 = 36) $$ choices for each of the $$ 5 $$ places, is
...
This value is known as the factorial and is represented by $$ n! $$ This [video](https://www.youtube.com/watch?v=Mfk_L4Nx2ZI&pp=0gcJCdgAo7VqN5tD) has a cool explanation
why $$ 0! $$ equals $$ 1 $$.
...
till $$ 32 $$ for the fifth and final character. So the total number of possible passwords
would be $$ 36 \times 35 \times 34 \times 33 \times 32 $$. This result is equivalent to 
$$ 36! $$ divided by $$ 31! $$ We call this a _permutation_,
where the order of the objects matter (ordered sampling without replacement). Simply put, it is _the total number of possible arrangements_ of $$ n $$ objects taken $$ k $$ at a time, with
...
This doesn't seem very intuitive at first glance, so let's demonstrate this with an example.
...
This can also be considered as unordered sampling without replacement.
...
Even though there are 6 possible ways for getting both sums, a sum of 10 is more likely than a
sum of 9 because the presence of identical elements in an event reduces
the total number of permutations.

In both cases, there are 3 events where all
the numbers are unique. This gives a total of $$ 3! \times 3 = 18 $$ possible arrangements for both.
...
possible arrangements. For the sum of 10, all the remaining three events 
...
from our set for each of our slots. Since there are $$ 2 $$ options and $$ n $$ objects,
it gives us a total of $$ 2^n $$ possible subsets including the null subset where no objects
are chosen. If you've used bitmasking in programming, this should be something you're already familiar with.
...
The Inclusion-Exclusion principle is an extremely powerful technique that allows us to find the union
...
to take care of the overlap during counting. If we were to extend this idea to $$ 3 $$ sets,
we would first add all the single sets, then subtract all the combinations of the intersection of
...
A great example of applying our inclusion-exclusion knowledge can be found
when we're asked to count the number of onto functions
(functions where range = co-domain, in other words every element in the co-domain must be
...
where no element go to their original slot. Let's consider a
set $$ A_i $$ as the permutation where the i-th element is fixed in its original position.
Taking an example where we apply derangement to $$ 4 $$ objects,
we first calculate sum of the single sets where one object is fixed. There are
...
$$ 5 $$ chocolates ($$ n = 5 $$) to $$ 3 $$ ($$ k = 3 $$) kids. One possible solution could be:
...
For each such solution, we always place $$ n $$ chocolates and $$ k - 1 $$ bars for the partitions, which gives us
a total of $$ n + k - 1 $$ positions. For this, we only need to choose $$ k - 1 $$ positions for the bars and the rest
of the positions will be assigned to the chocolates (items). This gives us
$$ \binom{n + k - 1}{k - 1} = \binom{n + k - 1}{n} $$. It is equivalent to arranging 
(or grouping) $$ n + k - 1 $$ items using the multinomial coefficient
where $$ n $$ items are of one kind and $$ k - 1 $$ are of another.

$$
\frac{(n + k - 1)!}{(k - 1)! \times n!}
$$
...
where the first kid must be given at least $$ 1 $$ chocolate and the second kid must have at least $$ 2 $$
(Also, how many ways can we get a sum of 10
...
If the constraint on $$ x_1 $$ was $$ x_1 \gt 1 $$, then $$ x_1' = x_1 - 2 $$ since 2 is the smallest possible
value $$ x_1 $$ can take.

Let's consider the equation with the following constraints:
...
x_1 > 3 + 1; \quad x_2 > 4 + 1; \quad x_3 > 2 + 1; \quad x_4 > 6 + 1
...
The case of identical slots is rather cumbersome. Let's take the scenario where we have 6 distinct books
...
elements in $$ (n - 1)! $$ ways taking the first item as reference.
Thus, the number of circular permutations of $$ n $$ objects become $$ (n - 1)! $$

An interesting case of circular arrangements arise from the arrangement of beads in a necklace. Apart
from the usual rotational symmetry,
the reflectional symmetry is also considered where the necklace is flipped
along the y-axis. This means that half of the unique arrangements that we get from a normal
...
(rotation/reflection). Let's take the example of a necklace with 4 beads and 2 colours (represented by 'A' and 'B')
...
the colourings that stay the same are `AAAA`, `ABAB`, `BABA`, `BBBB` and its fix is $$ 4 $$. If we do this
for all transformations and divide the sum of the all fixes by the number of transformations ($$ 8 $$),
we get the number of orbitals or in other words, the number of distinct colourings that are asymmetric.
...
(e.g. $$ 90\degree $$ and $$ 270\degree $$)

This lemma finds its use in unlabelled graph enumeration problems and even in
chemistry to count isomers of a compound.

> It can also be used to solve the problem from the Distinct Slots section in a similar way!
