...
There are imbalances present everywhere in nature. Their very existence is a macrocosm of bizarre phenomena 
...
trivial solutions through a series logical consequences that arise from inequalities. And much to the irony,
...
comprehensible and intuitive with a greater on focus on their applications, but I certainly do not mind
...
_It states that for any non-negative real number, their AM is greater than their GM, which is in trun greater
...
the averaging of compounding effects. To illustrate this, let us consider the annual returns of an investment
for $$ 3 $$ years to be $$ 5\% $$, $$ 8\% $$, and $$ 6\% $$ respectively.

If formula for the average growth rate for the period is given by
...
The signifance of this lies in the fact that adding $$ 1 $$ accounts for the initial value as well as the growth
rate. Let's take an example. Say an investment has grown by $$ 12\% $$ (growth rate) in the past year.
...
accurate in averaging rates with data whose relationships are multiplicative or proprotional in nature, as in our
case.
...
The simplest example can be found in averaging speeds.
...
to $$ \frac{total distance}{total time} $$. If we now calculate the average speed, we'll get $$ 26.66 $$ which is equal
to the harmonic mean of the speeds and not the AM.
...
Why is this true? It is because the harmonic mean of speeds is weighted by time and not distance. Speed is inversely
related to time. Hence, the time we spend at a particular speed affects the overall speed. The slower
...
with equality holding iff the vectors are linearly dependent (there exists a scalar $$ a $$ which satisfies $$ \vec{v} = a\vec{u} $$)
...
For the sake of simplicity, I have considered the vectors to belong to the real space $$ \mathbb{R}^n $$. But in general, 
it holds true in non-Euclidean vector spaces with complex forms. The dot product is just a special
...
This bound is not definitive but only provides a
conservative estimate which works well for many cases, regardless of the type of distribution. 
Due to this, it is widely applied in many statistical 
inferences. Let's demonstrate this with an example. 
...
To solve this, let's first calculate the $$ n $$ (also the z-score), which gives us the 
number of standard deviations above or below the mean. In this case, $$ \frac{60 - 30}{15} = \frac{90 - 60}{15} = 2 $$.
If the values of n were not equal, we would settle with the greater z-score (we are only concerened about the magnitude and not sign).
...
To understand Jensen's inequality, let us first understand convex and concave functions.
...
the functions concavity (or convexity) changes.
...
$$ f(x) $$ is concave up. Thus, by using the inequality, for all $$ x > 0 $$,
...
rule 1. Similarly, for the denominator, we know $$ log\_{e^1/n}(e) = n \times log_e(e) = n $$ as $$ log(e) = 1 $$.
...
Mathematics was always my top subject throughout school. I never participated in rigorous
maths competitions or bagged accolades of excellence, but my fascination for the discipline has no upper-bounds. 
...
Embrace, not supress the imbalances that wrestle inside you. They all exist for a reason. 
