...
no end to the errors that would pop up on my editor whenever I opened a *.ts* file. And more often than not, I ended
...
> Although I will review fundamentals whenever necessary, this is not a TypeScript tutorial! I 
> assume you have basic familiarity with TypeScript.
...
function studentInfo<T extends Student, P extends keyof T>(student: T, extra: P): string {
    return `The student's name is ${student.name} and his ${String(extra)} is/are ${student[extra]}`
}
...
Please note the distinction between *types* and *values* in TypeScript. This is an extremely common pitfall
encountered by beginners. Generics only accept *types* and not *values*, like most other operations in TypeScript
...
function studentInfo<T extends Student, P extends (typeof specialities)[number]>(student: T, speciality: P): string {
