...
The latter is one of the biggest benefits of rebasing. Merging multiple branches (through merge commits) can
create a messy and convoluted commit graph.
...
Often, the easiest way to update our private feature branch with the latest commits 
of the main branch is to rebase it using the powers of git pull (`git pull --rebase origin main`). This 
will not create a merge commit as it normally would without the `--rebase` flag, but instead fetch 
the latest commits from the main branch and apply our commits on top of it. I find myself 
using this command extensively.
...
> [git rebase --onto](https://dineshpandiyan.com/blog/stacked-diffs-with-rebase-onto/) is a powerful
> command that can be used to rebase layered branches properly.
