...
They will shred your silly commit messages, infiltrate your branches and plunder you of hundreds of lines
of your spaghetti code, not to mention time. The only way you get to combat your those that stand your way is to befriend Git.
In this article, I will go over some crucial Git concepts and techniques that I've picked up over the last year.

As always, this is not meant to be an introduction to Git. I assume your familiar with preliminary Git stuff
...
Primarily, developers hover around two philosophies when it comes to updating branches. **Git Merge** and
**Git Rebase**.
Merge creates a new _merge commit_ that merges all the changes from the feature branch
(the branch you want to merge) to the parent branch (the branch you are on) into a single
...
A Fast-Forward merge (FF-merge) occurs when the parent branch has not diverged from the feature branch, in other words, it
does not contain new commits since the base commit (parent commit) of the feature branch. Conventionally, a
normal merge commit is created when both the parent branch and the feature branch have diverged.
...
whenever possible or a normal merge otherwise. But for specific situations, we can have various flags at our
...
When working with remote branches, there are two commonly used methods for merge. Using `git pull`
...
branches changes the base commit of our feature branch to the latest commit on the parent branch, thus updating
it with the latest commits from its parent branch and creating a linear timeline that's easy to understand.
...
main branch, which leads to the main branch incorporating the commits made in the feature branch as its own.
...
Evidently, the higher number of unique commits, the greater the hassle. But please note that merge conflicts
only arise when there are discrepancies between two versions in the same file. If the main branch does not have
any commits that have modified the file the commits in your feature branch have, there will be no
merge conflicts! You can abort rebase using
the `--abort` flag or `--skip` to skip a commit.

A crucial feature about rebasing that is often overlooked is that the commit hashes change
when a branch is rebased. This is because rebasing, like we discussed, re-writes history. Since the base commit
changes, it also influences the change in the hashes of individual commits. This also allows us to hange the
commit message of each commit in the feature branch during the process.

> We should refrain from rebasing pushed commits. As commit hashes change when rebasing, it causes issues for
> other developers you're collaborating with.

## Editing with Interactive Rebase

Discovering `git rebase --interactive` felt like a game changer. Interactive Rebase is an extremely versatile
and powerful tool that allows us to modify our worktree.
...
We can take a cleaner look at our a commit history using `git log --oneline`, which does away with information
like date/time of commit and author.
...
sorted in their order of occurence, with the lastest commit being at the bottom.

`pick` is the default action for interactive rebase. All commits are set to `pick` as the rebase consider
every commit. To squash commits 2 and 3 together into a single commit, we can change the `pick` to `squash`
...
Edit is used for editing a contents of commit during rebase. This will instruct Git to pause when it reaches the
...
- `git stash list` - Displays a list of all stashes. (You can have multiple stashes) It
  is usually followed by the index `stash@{number}`. The lastest stash has the index of 0.

- `git stash apply <stash-index>` - Applies the specified stash to the workspace. If the index is not
  specified, the lastest stash is applied.
...
A detached head state is when the `HEAD` of a branch is not pointing directly to any branch but rather
...
branch labels associated with the commit. Instead, the changes can be commited and then accessed later by saving
...
errors and go back to a previous safe state. In this section, we'll discuss the most common methods to retrace
to previous commits.

#### Git Reset

Reset comes in two flavours. `--soft` and `--hard`. Using `--soft` keeps the file changes in the workspace but
only moves the `HEAD` pointer to a previous commit. We must a commit-hash or the `HEAD` syntax to reset to a
particular commit. For example, `--soft HEAD~2` will take the pointer to the last
but second commit. Whereas, `--hard HEAD~2` will reset the file changes in the workspace to that commit while
also moving the `HEAD` backwards. The `HEAD~n` and `<commit-hash>` syntax can be used in place of the other.

`--soft` can be used to "delete" commit messages and create new ones without altering the workspace.
...
room for merge conflicts. It simply undos all current changes and brings the workspace back to a previous commit.
Hence it is pretty straightforward. But `revert` allows us to consider and compare both current and previous
states, hence creating a merge conflict. The same flags from rebase can be used to complete the reversion
accordingly.

Another significant variation between the two is how they reflect the changes to particular commit.
...
the workspace (and HEAD pointer) will be brought back to the state after second commit, which is commit2.
...
the workspace will revert back to the state that was present before the second commit (commit2) was made.
Effectively, putting us back in state after the first commit was created. A major advantage of `revert` is the
...
One of the major downsides to rebasing is the when you choose to undo rebased branches. Although, it is not
encouraged to undo rebases as it makes matters more complicated for people working on them same or adjoining
branches. Since rebasing changes commit hashes and re-writes history, it is often a problem when old commits are
referenced.
