...
![Vim wizard - personal art](https://res.cloudinary.com/dul5f1mem/image/upload/v1709673911/blog/wiz_innuoh.webp)
...
text without the need of a mouse. Now, it is not compulsory for you to use Vim as your text-editor to learn Vim
...
Unlike other editors, Vim has modes, which are perspectives that segregates the actions of editing and text input
,with the ability to switch between them as necessary. The most important ones are -
...
Vim motions started to make way more sense once I realized they could be deconstructed as lexical items
in a human language. It is based on a meticulous system of _nouns_, _verbs_ and _movements_
...
current line. Similarly, `Fg` moves the cursor backwards to the position of the first occurence of the character
...
We can substitute any _verb_ with a _movement_ to carry out an action. We can also include a _number_ after
the _verb_ to specify the length of characters or lines that will be modified by the action. For example, `d4j`
deletes 4 lines down from the current cursor position, since `j` is the downwards
movement key.
...
3. If you need to jump to a word that's a few words down the line, but don't want to bother having to hold
   ...
   is before your current cursor position, replace `/` with `?`
...
6. `bi` to insert infront of the current word and `ea` to insert behind it. Priceless move.
...
9. I knew I wasn't a newbie anymore when I noticed that performing a delete or change operation is equivalent to
   cutting and that there wasn't separate key combinations for each.

10. Marking is easily the coolest feature in Vim. It works a little like macros by mapping a character to a
...
I compiled a non-exhaustive list of useful Vim commands in this [Github Repository]().
...
it induces a subtle restlessness in your fingers once you start getting the hang of it. Something that doesn't wear off
easily. At least, not when you're typing over a hundred words every day. Perhaps slowing down will save you the
dread of losing your phalanges over time, or maybe it's just me. I haven't a clue. But nevertheless, I've loved
very minute I've Vim-med
and there's just no going back. Remember, it demands a fair bit of practice before you find yourself whizzing
through your code-base like a golden snitch. So crack your knuckles and Vim on!
